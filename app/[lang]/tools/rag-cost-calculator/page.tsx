      ...
      <section className="border-t border-[var(--border)] bg-[var(--secondary)]/30">
        <div className="mx-auto max-w-4xl space-y-10 px-6 py-16">
          <article className="prose prose-sm max-w-none text-[var(--muted-foreground)]">
            ...
            <p>
              {lang === "fr"
                ? "Un système RAG (Retrieval-Augmented Generation) combine une base de données vectorielle avec un modèle de langage pour interroger vos documents internes. Le coût total dépend de trois postes : l'indexing initial des documents (embeddings), les tokens input consommés à chaque requête (question + contexte récupéré), et les tokens output générés par le LLM (réponse). Ce calculateur estime ces trois composantes en temps réel selon les tarifs officiels GPT-4o, GPT-4.5, Claude Sonnet 4.6, Claude Opus 4.6 et Mistral."
                : "A RAG (Retrieval-Augmented Generation) system combines a vector database with a language model to query your internal documents. Total cost breaks down into three components: initial document indexing (embeddings), input tokens consumed per query (question + retrieved context), and output tokens generated by the LLM (response). This calculator estimates all three in real time using official GPT-4o, GPT-4.5, Claude Sonnet 4.6, Claude Opus 4.6 and Mistral pricing."}
            </p>
